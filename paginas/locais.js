...
function Locais({route, navigation}) {
  ...
  useEffect(() => {
    if (param1 == 'Serviços Públicos') {
    ...
    } else if (param1 == 'Saúde') {
      fetch(
        'https://geo.cm-viana-castelo.pt/arcgis/rest/services/Viana_acessivel/MapServer/1/query?where=1%3D1+and+CATEGORIA%3D%27Sa%C3%BAde%27&text=&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&relationParam=&outFields=OBJECTID+%2CCATEGORIA%2C+FOTO%2C+DESCRICAO%2C+DESIGNACAO&returnGeometry=true&returnTrueCurves=false&maxAllowableOffset=&geometryPrecision=&outSR=&returnIdsOnly=false&returnCountOnly=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&returnZ=false&returnM=false&gdbVersion=&returnDistinctValues=false&resultOffset=&resultRecordCount=&queryByDistance=&returnExtentsOnly=false&datumTransformation=&parameterValues=&rangeValues=&f=pjson',
      )
        .then((response) => response.json())
        .then((responseJson) => {
          setData(responseJson.features);
      ...
      fetch(
        'https://geo.cm-viana-castelo.pt/arcgis/rest/services/Viana_acessivel/MapServer/1/query?where=1%3D1+and+CATEGORIA%3D%27' +
          param1 +
  ...
  // const loadFallback = () => {
  //console.log("deu erro esta imagem")
  //}
  ...
  return (
    <View
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <View>
            <TouchableOpacity style={styles.espaco}></TouchableOpacity>
            <TouchableOpacity
              style={styles.cartoes}
              onPress={() =>
                navigation.navigate('info', {param1: item.attributes.OBJECTID})
              }>
